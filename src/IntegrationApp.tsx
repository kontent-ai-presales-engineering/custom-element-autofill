  ...
  const updateElement = useDebouncedCallback(() =>
    deliveryClient
      ...
      .then(response => {
        ...
        const value = elements?.[config.sourceElement]?.value == elements?.[config.targetElement]?.value ? "" : elements?.[config.sourceElement]?.value; // Correct element access
        const name = config.nameElement && elements?.[config.nameElement]?.value == item.name ? "" : item.name; // Correct element access
